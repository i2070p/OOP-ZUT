...
int* createMarksArray(int mark1, int mark2, int mark3, int mark4, int mark5, int mark6, int mark7, int mark8, int mark9, int mark10){
   ...
   marks[0]=mark1; marks[1]=mark2; marks[2]=mark3; marks[3]=mark4;
   marks[4]=mark5; marks[5]=mark6; marks[6]=mark7; marks[7]=mark8;
   marks[8]=mark9; marks[9]=mark10;
   return(marks);
}
...
initial(_initial), studentId(_studentId), groupId(_groupId), marks(_marks) { changeNext(NULL); }
...
Group::Group(int _groupId): groupId(_groupId) {
	 head = NULL; 
	 tail = NULL; 
	 studentsCount=0; 
	 idMaker=0;
